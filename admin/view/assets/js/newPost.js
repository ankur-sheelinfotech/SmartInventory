...
jQuery(document).ready(function ($) {
    ...
    $('#newPost').on('click', '#btnSave', function (e) {
        ...
        let heroValue = $("input[name='heroRadios']:checked").val();

        // Validate all required fields (including images) but skip heroImage for now
        $('input[required]').each(function () {
            // If heroImage and heroRadios is "No", skip validation
            if ($(this).attr('id') === 'heroImage' && heroValue !== "Yes") {
                return true; // Continue to next iteration (skip validation)
            }

            // General validation for all other required fields
            if ($(this).val().trim() === '') {
                isValid = false;
                $(this).css('border-color', 'red'); // Highlight invalid fields
            } else {
                $(this).css('border-color', ''); // Remove highlight
            }
        });

        // Show error message for Hero Image if "Yes" is selected but no image is provided
        if (heroValue === "Yes" && $("#heroImage").val().trim() === "") {
            isValid = false;
            $("#hero_image_error").text("Hero Image is required.").show();
            $("#heroImage").css('border-color', 'red');
        } else {
            $("#hero_image_error").hide();
            $("#heroImage").css('border-color', '');
        }
